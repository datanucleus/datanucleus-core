...
public class BTMTransactionManagerLocator extends FactoryBasedTransactionManagerLocator
{
    ...
    protected Class getFactoryClass(ClassLoaderResolver clr)
    {
        ...
        try
        {
            try
            {
                factoryClass = clr.classForName("bitronix.tm.TransactionManagerServices");
            }
            catch (Exception e)
            {
            }
        ...
        catch (Exception e)
        {
            ...
            {
                NucleusLogger.TRANSACTION.debug("Exception finding BTM transaction manager. " +
                    "Probably not in a BTM environment " + e.getMessage());
            }
        }
        return factoryClass;
    }
