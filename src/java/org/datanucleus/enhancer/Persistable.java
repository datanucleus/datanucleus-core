 ...
 * <P>
 * In the Reference Implementation, the JDO Enhancer modifies the class to implement PersistenceCapable prior
 * to loading the class into the runtime environment. The Reference Enhancer also adds code to implement the
 * methods defined by PersistenceCapable.
 * <P>
 * The extra methods in the PersistenceCapable interface might be generated by pre-processing a .java file, or
 * might be generated from a tool directly. The exact technique for generating the extra methods is not
 * specified by JDO.
 * <P>
 * The PersistenceCapable interface is designed to avoid name conflicts in the scope of user-defined classes.
 * All of its declared method names are prefixed with 'jdo'.
 */
public interface Persistable
{
    /**
     * If jdoFlags is set to READ_WRITE_OK, then the fields in the default fetch group can be accessed for
     * read or write without notifying the StateManager.
     */
    static final byte READ_WRITE_OK = 0;

    /**
     * If jdoFlags is set to LOAD_REQUIRED, then the fields in the default fetch group cannot be accessed for
     * read or write without notifying the StateManager.
     */
     ...
     * without notifying the StateManager.
     */
    ...
    /**
     * If jdoFieldFlags for a field includes MEDIATE_READ, then the field has been enhanced to always call the
     * jdoStateManager on all reads.
     */
    ...
    /**
     * If jdoFieldFlags for a field includes MEDIATE_WRITE, then the field has been enhanced to always call
     * the jdoStateManager on all writes.
     */
