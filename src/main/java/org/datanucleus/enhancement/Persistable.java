 ...
 * <P>
 * This interface defines methods that allow the implementation to manage the instances. 
 * It also defines methods that allow a DataNucleus aware application to examine the runtime state of instances. 
 * For example, an application can discover whether the instance is persistent, transactional, dirty, new, deleted, or detached; and to get its 
 * associated ExecutionContext, object identity, and version if it has one.
 * <P>
...
public interface Persistable
{
     ...
     * This method is normally used by the StateManager during the process of making an instance persistent, transient, or transactional. 
     * The caller of this method must have JDOPermission for the instance, if the instance is not already owned by a StateManager. 
     * If the parameter is null, and the StateManager approves the change, then the dnFlags field will be reset to READ_WRITE_OK. If the parameter is not null, 
     * and the security manager approves the change, then the dnFlags field will be reset to LOAD_REQUIRED.
     ...
     * Normally, Persistable classes are able to detect changes made to their fields. 
     * However, if a reference to an array is given to a method outside the class, and the array is modified, then the persistent instance is not aware of the change. 
     * This API allows the application to notify the instance that a change was made to a field.
     * <P>
     * The field name should be the fully qualified name, including package name and class name of the class declaring the field. 
     * This allows unambiguous identification of the field to be marked dirty. If multiple classes declare the same field, and if the package and class name are 
     * not provided by the parameter in this API, then the field marked dirty is the field declared by the most derived class.
     * <P>
     ...
     * <P>
     * The ObjectId may be serialized and later restored, and used with a ExecutionContext from the same DataNucleus instance to locate 
     * a persistent instance with the same data store identity.
     * <P>
    ...
    /**
     * Return a copy of the identity associated with this instance. 
     * This method is the same as dnGetObjectId if the identity of the instance has not changed in the current transaction.
     * <P>
     ...
     * Instances whose state is associated with the current transaction return true. Transient instances return false.
     ...
     * Instances that have been made persistent in the current transaction return true. Transient instances return false.
     ...
     * <P>
     * This method is used as a performance optimisation as an alternative to using reflection to construct a new instance. 
     * It is used by the EnhancementHelper class method newInstance.
     ...
     * <P>
     * This method is used as a performance optimisation as an alternative to using reflection to construct a new instance of a class that uses application identity. 
     * It is used by the EnhancementHelper class method newInstance.
     * @param sm the StateManager that will own the new instance.
     ...
     * This method creates a new instance of the class used for identity. It is intended only for application identity. 
     * If the class uses datastore identity, or if the class is abstract, null is returned.
     * <P>
     * For classes using single field identity, this method must be called on an instance of a persistence-capable class with its primary key field 
     * initialised (not null), or an exception is thrown.
     * <P>
     ...
     * It is intended for single field identity. The identity instance returned has no relationship with the values of the primary key fields 
     * of the persistence-capable instance on which the method is called. If the key is the wrong class for the object id class, null is returned.
     * <P>
     ...
     * <ul>
     * <li><code>Number</code> or <code>Character</code>: the parameter must be the single field type or the wrapper class of the primitive field type; 
     * the parameter is passed to the single field identity constructor</li>
     * <li><code>ObjectIdFieldSupplier</code>: the field value is fetched from the <code>ObjectIdFieldSupplier</code> and passed to the single field identity constructor</li>
     ...
     * This method is generated in the <code>Persistable</code> class to generate a call to the field manager for each key field in the ObjectId. 
     * For example, an ObjectId class that has three key fields <code>(int id, String name, and Float salary)</code> would have the method generated: 
     * <code>
     ...
     * </code>
     * <P>
     * The implementation is responsible for implementing the <code>ObjectIdFieldSupplier</code> to produce the values for the key fields.
     * <P>
     * For classes using single field identity, this method always throws <code>JDOUserException</code>.
     ...
     * </code>
     * <P>
     * The implementation is responsible for implementing the <code>ObjectIdFieldConsumer</code> to store the values for the key fields.
    ...
    /**
     * This interface is used to provide fields to the Object id instance. It is used by the method copyKeyFieldsToObjectId. 
     * When the method is called, the generated code calls the instance of the FieldManager for each field in the object id.
     */
    ...
    /**
     * This interface is used to store fields from the Object id instance. It is used by the method copyKeyFieldsFromObjectId. 
     * When the method is called, the generated code calls the instance of the FieldManager for each field in the object id.
     */
